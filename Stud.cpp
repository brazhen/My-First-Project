...
// Funkcija spausdinti studentų rezultatus vector
...
// Funkcija spausdinti studentų rezultatus list konteineryje
void spausdintiRezultatus(const list<Stud>& studentaiList, char skaiciavimo_budas) {
    cout << setw(20) << "Pavarde"
         << setw(20) << "Vardas"
         << setw(20);
    ...
    for (const Stud& s : studentaiList) {
        double galutinis;
        if (skaiciavimo_budas == 'v' || skaiciavimo_budas == 'V') {
            galutinis = galutinisVid(s);  // Naudojame vidurkį
        } else {
