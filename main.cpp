...
using StudentList = list<Stud>;    // List tipas
using StudentVector = vector<Stud>; // Vector tipas


 int main() {
      char konteineris; // Kiekvienas konteineris
    StudentList studentaiList;    // List tipo studentų konteineris
    StudentVector studentaiVector; // Vector tipo studentų konteineris
    ...
    do {
       ...
       if (pasirinkimas == 'g' || pasirinkimas == 'G') {
            ...
            // Failo nuskaitymas ir konteinerio pasirinkimas
            ...
            if (konteinerio_pasirinkimas == 'v' || konteinerio_pasirinkimas == 'V') {
                ...
                diff = end - start;
                cout << "Duomenu nuskaitymas uztruko (vector): " << diff.count() << " s" << endl;

                // Studentu rusiuoti i kategorijas
                start = chrono::high_resolution_clock::now();
                rusiutiStudentus(studentai);
                end = chrono::high_resolution_clock::now();
                diff = end - start;
                cout << "Studentu rusiuoti uztruko: " << diff.count() << " s" << endl;
            } else {
                list<Stud> studentaiList;
                start = chrono::high_resolution_clock::now();
                skaitytiIsFailoSuKlaiduTikrinimu(studentaiList, failo_pav);
                end = chrono::high_resolution_clock::now();
                diff = end - start;
                cout << "Duomenu nuskaitymas uztruko (list): " << diff.count() << " s" << endl;

                // Studentu rusiuoti i kategorijas
                start = chrono::high_resolution_clock::now();
                rusiutiStudentus(studentaiList);
                end = chrono::high_resolution_clock::now();
                diff = end - start;
                cout << "Studentu rusiuoti uztruko: " << diff.count() << " s" << endl;
            }
        ...
        if (konteineris == 'l') {
    spausdintiRezultatus(studentaiList, skaiciavimo_budas);  // `skaiciavimo_budas` kaip 'v' arba 'm'
} else {
    spausdintiRezultatus(studentaiVector, skaiciavimo_budas);
}
